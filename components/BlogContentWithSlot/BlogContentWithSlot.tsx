  ...
  if (isEditor) {
    return (
      <div className={className}>
        ...
        <div className={date?.style} style={{ color: '#666', marginBottom: '10px' }}>
          January 1, 2024
        </div>
        <div className={author?.style} style={{ color: '#666', marginBottom: '20px' }}>
          By Author Name
        </div>
        <div className={description?.style} style={{ lineHeight: '1.6', marginBottom: '30px' }}>
          This is a preview of how your blog post will appear. The actual content will load from
          Contentful on the live site.
        </div>
        ...
        <div style={{ marginTop: '2rem', padding: '1rem', background: '#f9f9f9', borderRadius: '8px', border: '1px dashed #ccc' }}>
  ...
  const BlogContentLive = React.lazy(() => {
    return import('@/lib/contentful/provider').then(({ useContentfulData }) => {
      const LiveComponent = () => {
        ...
        // Load icon config from localStorage
        useEffect(() => {
          ...
          try {
            ...
            if (parsed && typeof parsed === 'object') {
              if (parsed.pagination) {
                setPaginationConfig(prev => ({ ...prev, ...parsed.pagination }))
              }
              if (parsed.return) {
                setReturnConfig(prev => ({ ...prev, ...parsed.return }))
              }
            }
          } catch {
            // ignore
        ...
        const mergedPaginationConfig = { ...paginationConfig, ...(paginationIcon || {}) }
        const mergedReturnConfig = { ...returnConfig, ...(returnIcon || {}) }

        // ── Related content ──────────────────────────────────────────────
        const blog = Array.isArray(blogs) && blogs.length > 0 ? blogs[0] : null
        
        // Debug: Log what we're receiving from Contentful
        if (blog) {
          const relatedBlogPostsRaw = (blog as any)?.relatedBlogPosts
          const relatedProjectsRaw = (blog as any)?.relatedProjects
          console.log('[BlogContentWithSlot] Raw related data from Contentful:', {
            blogTitle: (blog as any)?.title,
            relatedBlogPostsRaw,
            relatedProjectsRaw,
            blogPostsType: typeof relatedBlogPostsRaw,
            projectsType: typeof relatedProjectsRaw,
          })
        }
        
        const relatedBlogSlugs = parseSlugs((blog as any)?.relatedBlogPosts)
        const relatedProjectSlugs = parseSlugs((blog as any)?.relatedProjects)
        
        console.log('[BlogContentWithSlot] Parsed slugs:', {
          relatedBlogSlugs,
          relatedProjectSlugs,
        })
        ...
        useEffect(() => {
          ;(async () => {
            ...
            try {
              ...
              const prevBlog =
                index >= 0 && index < all.length - 1 ? all[index + 1] : null
        ...
        const validBlogs = blogs.filter(
          (blog): blog is NonNullable<typeof blog> =>
            blog != null && blog.__typename === 'BlogPost' && typeof blog._id === 'string'
        )

        if (validBlogs.length === 0) return <div>No valid blog posts available</div>

        const currentBlog = validBlogs[0]

        console.log('BlogContentWithSlot - banner object:', currentBlog?.banner)

        return (
          <div className={className} id="blog-content">
            <a href="#blog-content" className="focusable-skip-link">
            ...
            <div className="breadcrumbs" style={{ marginBottom: '20px' }}>
              <a href="/">Home</a> &gt;
              <a href="/blog">Blog</a> &gt;
              <span>{currentBlog?.title || 'Blog Post'}</span>
            </div>
            ...
            <BlogPostRichText
              fieldPath="body"
              className={body?.style}
              bannerUrl={currentBlog?.banner?.url ?? undefined}
                  ...
                  {pagination.next ? (
                    ...
                    >
                      ...
                      <div className="text-right">
                        <div className="text-sm">Next Post</div>
                ...
                {!pagination.isNewest && pagination.newest && (
                  ...
                  >
                    <article className="group relative w-full h-full border rounded-lg p-4 flex flex-col gap-4 transition-transform duration-300 hover:scale-[1.02]">
                      ...
                      <div>
                        <div className="text-xs">Newest</div>
